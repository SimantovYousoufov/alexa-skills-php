...
class AlexaSessionEndedRequestTest extends TestCase
{
	public $config = [
	...
	public function testItAcceptsAndSetsDependencies()
	{
		...
		$alexa_request = new SessionEndedRequest($request, $this->config, $verifier, $session);
	...
	public function testItReturnsCorrectRequestType()
	{
		...
		$this->assertEquals('SessionEndedRequest', $alexa_request->requestType());
	}

	public function testItSetsAndGetsSessionEndedReason()
	{
		$session  = Mockery::mock(EphemeralSession::class);
		$verifier = Mockery::mock(RequestVerifier::class);
		$verifier->shouldReceive('verifyRequest');
		...
		$alexa_request->setEndedReason(SessionEndedRequest::REASON_ERROR);

		$this->assertEquals('REASON_ERROR', $alexa_request->reason());
	}
}
